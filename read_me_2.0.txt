NOTE: * Use the ports and passwords i have provided on slack *
Project Description:
Part One:
You are going to create a very simple and straightforward webpage that will be phase one of projects to come next semester. It will involve running a backend server using flask (instructions below), creating a mongoDB database with any WRF data that we have given you access to (recall https://github.com/ASRCxCITE/mongo_tutorial), creating any kind of plots and serving them on the front end (recall workshop https://github.com/ASRCxCITE/Tutorials). You can be creative and make it look nice or do the minimal and just show a single plot. Once you are done you will push your project to github. The readme must contain the URL so Kara and myself can access the front end and see the work you have done (example URL will look like http://coeml.asrc.albany.edu:5006/)
Part Two:
...
$ python app.py
Server should start
Access webpage from http://169.226.181.104:5006/ or http://coeml.asrc.albany.edu:5006/
...
http://coeml.asrc.albany.edu:5006/test
